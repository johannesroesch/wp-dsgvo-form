...
class AuditLogger {
	 ...
	 *
	 * PERF-SOLL-04: Uses batched DELETE (LIMIT 500) to avoid long-running
	...
	public function cleanup_old_entries(): int {
		...
		$table       = $wpdb->prefix . 'dsgvo_audit_log';
		$cutoff_date = current_time( 'mysql', true );
		$total       = 0;

		// PERF-SOLL-04: Batch-DELETE to prevent table locks on large datasets.
		do {
			// SEC-SQL-01: Prepared statement.
			$deleted = (int) $wpdb->query(
				$wpdb->prepare(
					"DELETE FROM `{$table}` WHERE `created_at` < DATE_SUB(%s, INTERVAL %d DAY) LIMIT 500",
					$cutoff_date,
